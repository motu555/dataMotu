...
public class ReviewFilterParser {
    ...
    final static int reviewIdStrLength = "\" data-id=\"".length();
    final static int userIdStrLength = "<a target=\"_blank\" href=\"/member/".length();

    final static String reviewIdStart = "\" data-id=\"";

    final static String userIdStart = "<a target=\"_blank\" href=\"/member/";
    final static String userIdEnd = "\">";
    ...
    public ReviewFilterParser(String dataPathParam, String cityCodeParam, String storePathParam, List<String> shopListParam) {
        ...
        this.reviewPath = this.dataPath + "/" + cityCode + "/review_filtered/";
    }

    public ReviewFilterParser() {
     ...
     */
    public void parseAllShopReview() throws IOException, ParserException {
        ...
        for (String shopId : shopList) {
            ...
            if (shopNum % shopNumProcess == 0) {
                ...
                FileOperation.writeAppdend(this.storePath + "/" + this.cityCode + "/review_filter.json", this.reviewJson.toString());
                FileOperation.writeAppdend("cache/" + cityCode + "reivewFilterParsed.txt", shopIdStr.toString());
                this.reviewJson = new StringBuilder("");
    ...
    /**
     * parse all shop reviews of a city cityCode  多线程版本
     * 手动将其分为4个多线程，加锁同步后写入同一个文件
     */
                ...
                public void run() {
                    ...
                    for (int j = start; j < end; j++) {
                        ...
                        if (processedShopNum % shopNumProcess == 0) {
                            ...
                            synchronized (ReviewFilterParser.class) {
                                FileOperation.writeAppdend(storePath + "/" + cityCode + "/review_filter.json", reviewJson.toString());
                                FileOperation.writeAppdend("cache/" + cityCode + "reivewFilterParsed.txt", shopIdStr.toString());
                            }
     ...
     * @param shopId
     */
    public void parseAllReview(String content, String shopId) throws ParserException {
        NodeList commentList = SimpleHtmlParser.parseNodeAttribtute(content, "data-id");
...
//        System.out.println(size);
     ...
     */
    public int parseAllReview(String content, String shopId, StringBuilder reviewJson) throws ParserException {
        NodeList commentList = SimpleHtmlParser.parseNodeAttribtute(content, "data-id");
        int size = commentList.size();
        int reviewNum = 0;
        for (int i = 0; i < size; i++) {
     ...
     * @param content
     */
    public String parseOneReview(String content, String shopId) throws ParserException {

        String[] ids = this.getIds(content);
        if (ids[0] == null) {
            System.out.println("ids[0]\t" + shopId);
            return null;
        }
     ...
     */
    public String[] getIds(String content) throws ParserException {
        String[] ids = new String[2];
        NodeList idNodes = SimpleHtmlParser.parseNodeAttribtute(content, "data-id");
        if (idNodes.size() > 0) {
            String idsStr = idNodes.elementAt(0).getText();
//            System.out.println(idsStr);
            ids[0] = idsStr.substring(idsStr.indexOf(reviewIdStart) + reviewIdStrLength, idsStr.length() - 1);
//            System.out.println(ids[0]);
        } else {
            ids[0] = null;
        }

        NodeList idUserNodes = SimpleHtmlParser.parserHtml(content, "div", "class", "pic");
        if (idUserNodes.size() > 0) {
            String idsStr = idUserNodes.elementAt(0).toHtml();
            idUserNodes = SimpleHtmlParser.parserHtml(idsStr, "a", "target", "_blank");
            if (idUserNodes.size() > 0) {
                idsStr = idUserNodes.elementAt(0).toHtml();
//                System.out.println(idsStr);
...
//        System.out.println(idsStr);
    ...
    public int[] getRatings(String content) {
        ...
        if (ratingsNodes.size() > 0) {
            for (int i = 0; i < ratingsNodes.size(); i++) {
                String subContent = ratingsNodes.elementAt(i).toPlainTextString();
    ...
    public int getThumbs(String content) {
        int thumbs = -1;
        NodeList thumbsNodes = SimpleHtmlParser.parserHtml(content, "span", "class", "count");
